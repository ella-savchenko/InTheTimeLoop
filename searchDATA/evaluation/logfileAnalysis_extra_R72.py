...
def only_it(filename):
    log = open(filename, "r", encoding='utf-8', errors='ignore')
    iterations = 0
    for line in log:
...
def analyse_case(file_name):
    ...
    it_only = only_it(file_name)


    for i in range(0, it_only):
        case_R24.append([])
    ...
    for line in log:
        ...
        if "done search grep A R24" in line and "done search grep A R24-288" not in line:
            R24_b = False

        if "done search grep A R24-288" in line:
            R24_total_b = True
            case_R24[iterations].append(R24)
            R24 = []
            for i in range(0, 288):
                R24.append([])
        ...
        if (R24_b and (not R24_total_b)):
            #welches R -> R24 [R24-1, R24-2, ...R-24-96]
            line = line.replace(" ", "")
        ...
        if "search grep A R24" in line and "done" not in line:
            R24_b = True
            number = line
            parts = number.split('-')
            n= int(parts[1])

        if "search grep A R24-1" in line and "done" not in line:
            R24_total_b = False
    ...
    calculate_amounts_into_csv(case_a, case_R1, case_R2, case_R3, case_R4, case_R5, case_R6, case_W1, case_W2, case_W3, case_R24, start_date)
...
def calculate_amounts_into_csv(a, R1, R2, R3, R4, R5, R6, W1, W2, W3, R24, start_date):
    name = "results_R72/numbers_patterns.csv"
    start_date = start_date.strip()
    ...
    with open(name, mode='a') as csv_results:
        ...
        for i in range(len(a)):
            ...
            for k in range(len(R24[i][0])):
                n = k+1
                str_to_write = iteration + ',R24-'+str(n)+',' + str(len(R24[i][0][k])) + '\n'
                csv_results.write(str_to_write)
